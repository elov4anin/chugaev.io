...
var buildFolder = 'payment/build';/*Папка сборки*/
...
gulp.task("symbols", function () {
    return gulp.src(buildFolder +  "/images/icons-svg/*.svg")
	    .pipe(svgmin())
	    .pipe(svgstore({
			inlineSvg:true
		    }
...
gulp.task('svgSpriteBuild', function () {
    ...
    // minify svg
		...
		mode: {
			...
			render: {
			    styl: {
				dest:'../../../../../stylesheets/_sprite.styl',
				template: "stylesheets/_sprite_template.styl"
			    }
			}
		    }
		}
	    }))
	    .pipe(gulp.dest(buildFolder + "/images/sprite"));
});
...
gulp.task("build", function (fn) {
    run(
	  ...
	  /*  "sprite",*/
	   ...
	   /*   "svgSpriteBuild", */  /*сборка svg спрайтов не используется в проекте*/
	    "serve",
	    fn
    );
});
